...
// --- FEATURES HERO (2 COLONNES STRICTES) ---
const FeaturesHero = ({ language }: { language: Language }) => (
  <div className="max-w-6xl mx-auto px-4 py-8 mb-4">
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
      ...
      {/* 2 Colonnes pour afficher les cartes */}
...
const ProductCard = ({ 
  ...
  // FONCTION DE CLICK QUI GÈRE LE TRACKING ET LA REDIRECTION
  const handleWhatsAppClick = () => {
    // 1. Envoyer le tracking
  ...
  return (
    // LA DIV PRINCIPALE EST MAINTENANT CLIQUABLE POUR MAXIMISER LE CTR
    <div 
      onClick={handleWhatsAppClick}
      className="bg-white rounded-xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-300 cursor-pointer group"
    >
      ...
      <div className="p-4">
        ...
        <div className="text-center mb-6">
          <p className="text-4xl font-extrabold text-gray-900 leading-none group-hover:scale-105 transition-transform">
            {product.price}
            <span className="text-xl ml-1 text-gray-600">{language === 'ar' ? 'درهم' : 'DH'}</span>
          </p>
        </div>

        {/* Le bouton est visuel seulement, le clic est géré par la Div parente */}
        <button
          className={`w-full bg-gradient-to-r from-[#25D366] via-[#20bd5a] to-[#1da851] text-white font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2.5 transition-all duration-300 active:scale-95 relative overflow-hidden group/btn ${language === 'ar' ? 'flex-row-reverse' : ''}`}
          style={{
        ...
        >
          ...
          <svg className="w-5 h-5 opacity-80 group-hover/btn:translate-x-1 transition-transform relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
