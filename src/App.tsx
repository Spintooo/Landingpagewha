...
// --- COMPOSANT: HERO SECTION (REFONTE TOTALE) ---
  ...
  <div className="relative max-w-5xl mx-auto px-3 py-5">
    ...
    <div className="relative bg-[#064e3b] rounded-2xl overflow-hidden shadow-2xl border border-[#0f765e]">
      ...
      <div className="relative z-10 p-5">
        ...
        {/* Grid Features (Style Cartes Flottantes) */}
        <div className="grid grid-cols-2 gap-3 max-w-2xl mx-auto mb-4">
          ...
          <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-xl p-3 flex items-center gap-3 border border-white/10 shadow-lg transform hover:scale-[1.02] transition-transform">
            <div className="w-10 h-10 rounded-full bg-[#fbbf24] flex items-center justify-center text-xl shadow-inner shrink-0">
              🤝
            </div>
            <div className="text-right flex-1">
              <p className="text-white font-bold text-sm leading-tight">
                {language === 'ar' ? 'الدفع بعد الاستلام' : 'Paiement à la livraison'}
              </p>
              <p className="text-white/60 text-[10px]">
                {language === 'ar' ? 'عاين سلعتك عاد خلص' : 'Vérifiez avant de payer'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-xl p-3 flex items-center gap-3 border border-white/10 shadow-lg transform hover:scale-[1.02] transition-transform">
            ...
            <div className="text-right flex-1">
              <p className="text-white font-bold text-sm leading-tight">
                {language === 'ar' ? 'توصيل سريع ومجاني' : 'Livraison Rapide Gratuite'}
              </p>
              <p className="text-white/60 text-[10px]">
                {language === 'ar' ? 'إلى جميع مدن المغرب' : 'Partout au Maroc'}
              </p>
            </div>
          </div>
        </div>

        {/* Scroll Animation Clean */}
...
const ProductCard = ({ 
  ...
  return (
    // BORDURE GRIS FONCÉ (border-gray-300) ET OMBRE
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-300 transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div className="p-4">
        ...
        >
          ...
          <span className="text-[15px] tracking-wide uppercase">{ctaText}</span>
        </button>

        {/* SOCIAL PROOF */}
        <div className="mt-3 text-center">
          <p className="text-[11px] text-gray-500 font-medium">
            <span className="text-green-600 font-bold">+{product.soldCount}</span> {language === 'ar' ? 'عميل اشترى هذا الموديل' : 'clients ont acheté ce modèle'}
          </p>
        </div>
      </div>
    </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    if (fbclid) setTrackingData({ fbclid });
    ...
    shuffled.forEach(product => {
      window.fbq('track', 'ViewContent', { content_name: product.name.ar, content_ids: [product.id], value: product.price, currency: 'MAD' });
    });
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] flex flex-col font-sans" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <style>{`body { font-family: ${language === 'ar' ? "'Cairo', 'Almarai', sans-serif" : "system-ui, sans-serif"}; }`}</style>

      {/* HEADER AVEC LOGIQUE DE POSITIONNEMENT */}
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-lg shadow-sm border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-center relative">
          ...
          {/* BOUTON LANGUE - Position absolue selon la langue */}
          <button
